...
jQuery.validator.addMethod("proposta", function (value, element) {
	...
	return false;
}, "Informe o numero da proposta."); // Mensagem padrão
...
jQuery.validator.addMethod("tituloEleitoral", function (value, element) {

    value = pad(jQuery.trim(value), 12);    

	if (value) {
		var paddedInsc = value;
	...
	return true;
}, "Informe um título eleitoral válido."); // Mensagem padrão
...
jQuery.validator.addMethod("cpfcnpj", function (cpfcnpj, element) {
    if (jQuery.trim(cpfcnpj)) {
        var cnpjValido = false;
        var cpfValido = false;

        // CNJP
        cnpj = jQuery.trim(cpfcnpj);
...
jQuery.validator.addMethod("moneyPt", function (value, element) {
    if (value == 0)
    {
        return true;
    }
...
}, "Dinheiro."); // Mensagem padrão 
jQuery.validator.addMethod("decimalPt", function (value, element) {
    return this.optional(element) || /^[+-]?((\d+|\d{1,3}(\.\d{3})+)(\,\d*)?|\,\d+)$/.test(value);
}, "Decimal."); // Mensagem padrão 
jQuery.validator.addMethod("decimalEn", function (value, element) {
    return this.optional(element) || /^[+-]?((\d+|\d{1,3}(\,\d{3})+)(\.\d*)?|\.\d+)$/.test(value);
}, "Decimal."); // Mensagem padrão

jQuery.validator.addMethod("timeBR", function (value, element) {
    var isValid = /^([0-1]?[0-9]|2[0-3]):([0-5][0-9])(:[0-5][0-9])?$/.test(value);
    return isValid;
}, "Informe uma hora válida");

function ErrorLabel(errorMsg, element, errorElement) {

    //if (errorMsg == null) {
    //    errorMsg = "";
    //}

    //var error = $("<label class='error labelError'>" + errorMsg + "</label>");
    ...
    element.parents(".ui-btn").addClass("error");

    //var element = $(element);

    //if (element.parent()[0].tagName == "TD" && element[0].type == "radio") {
    //    var table = element.parent().parent().parent().parent();
    //    table.css("margin-top", "14px");
    //    error.insertBefore(table);
    //}
    //else {
    //    //element.css("margin-top", "15px");
    //    var $span = $("<span style='padding:0px !important;margin:0px !important;position:relative;'/>");
    //    element.wrap($span);
    ...
    //    var errorlabel = error.wrap('<panel></panel>');
    //    errorlabel.css("position", "absolute");
    //    errorlabel.css("width", "width:auto;");
